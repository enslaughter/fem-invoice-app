...
function InvoiceMenu(props: any) {
  ...
  function invoiceNumberText() {
    if (windowSize.width < 620) {
      return `${props.invoiceCount} Invoice${
        props.invoiceCount === 1 ? "" : "s"
      }`;
    } else {
      ...
      <div className="invoice-menu-right">
        <div className="invoice-menu-filters">
          <button onClick={toggleFiltersOpen}>
            Filter {windowSize.width > 620 ? "by status" : ""}
            <img
              className="downarrow"
              src={arrowdown}
          ...
          <div
            className="invoice-menu-filters-popup"
            style={
              filtersOpen ? { visibility: "visible" } : { visibility: "hidden" }
            }
          >
